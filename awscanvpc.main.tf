...
# Pushing both private keys to AWS Secrects Manager
resource "aws_secretsmanager_secret" "jenkins_mgr_key" {
  depends_on = [
    tls_private_key.jenkins_mgr
  ]
  name = "${var.resource_name_prefix}-mgr-privkey-${formatdate("YYYYMMDDhhmm", timestamp())}"
}

resource "aws_secretsmanager_secret_version" "jenkins_mgr_key_version" {
  depends_on = [
    aws_secretsmanager_secret.jenkins_mgr_key
  ]
  secret_id     = aws_secretsmanager_secret.jenkins_mgr_key.id
  secret_string = tls_private_key.jenkins_mgr.private_key_pem
}

resource "aws_secretsmanager_secret" "windows_agents_key" {
  depends_on = [
    tls_private_key.windows_agents
  ]
  name = "${var.resource_name_prefix}-agents-privkey-${formatdate("YYYYMMDDhhmm", timestamp())}"
}

resource "aws_secretsmanager_secret_version" "windows_agents_key_version" {
  depends_on = [
    aws_secretsmanager_secret.windows_agents_key
  ]
  secret_id     = aws_secretsmanager_secret.windows_agents_key.id
  secret_string = tls_private_key.windows_agents.private_key_pem
}

# Pushing various secrets to AWS Secrets Manager
resource "aws_secretsmanager_secret" "jenkins_admin_password" {
  depends_on = [
    aws_instance.jenkins
  ]
  name = "jenkins-admin-password-${formatdate("YYYYMMDDhhmm", timestamp())}"
}

resource "aws_secretsmanager_secret_version" "jenkins_admin_key_version" {
  depends_on = [
    aws_secretsmanager_secret.jenkins_admin_password
  ]
  secret_id     = aws_secretsmanager_secret.jenkins_admin_password.id
  secret_string = var.jenkinspassword
}

resource "aws_secretsmanager_secret" "windows_agent_admin_password" {
  depends_on = [
    aws_instance.jenkins
  ]
  name = "windows-agent-admin-password-${formatdate("YYYYMMDDhhmm", timestamp())}"
}

resource "aws_secretsmanager_secret_version" "windows_agent_key_version" {
  depends_on = [
    aws_secretsmanager_secret.windows_agent_admin_password
  ]
  secret_id     = aws_secretsmanager_secret.windows_agent_admin_password.id
  secret_string = var.agentadminpassword
}
